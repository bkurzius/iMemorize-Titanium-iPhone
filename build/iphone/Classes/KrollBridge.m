...
NSString * iMemorizeMobile$ModuleRequireFormat = @"(function(exports){"
...
@implementation iMemorizeMobileObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_imemorizemobile);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create iMemorizeMobile global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "iMemorizeMobile" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_imemorizemobile = [[iMemorizeMobileObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_imemorizemobile];
	
	NSString *_imemorizemobileNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _imemorizemobileNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_imemorizemobile valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:iMemorizeMobile$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.imemorizemobile.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.imemorizemobile.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}

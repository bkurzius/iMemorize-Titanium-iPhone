...
//Rare exceptions to indicate a bug in the _imemorizemobile code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_imemorizemobile = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_imemorizemobile]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.imemorizemobile.module"
